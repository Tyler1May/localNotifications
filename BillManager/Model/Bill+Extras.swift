...
extension Bill {
    ...
    mutating func scheduleReminders(on date: Date, completion: @escaping (Bill) -> ()) {
        ...
        authorizeIfNeeded { (granted) in
            ...
            let triggerDateComponets = Calendar.current.dateComponents([.second, .minute, .hour, .day, .month, .year], from: date)
            let trigger = UNCalendarNotificationTrigger(dateMatching: triggerDateComponets, repeats: false)
